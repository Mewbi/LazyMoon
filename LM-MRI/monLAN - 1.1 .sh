...
less << END
 ...
 -e|--registrarEmail)
	Registra o email que deverá ser uilizado para envios e relatorios.

 -p|--registrarPrioritarios)
	Registra o dispositivos prioritários.

 -s|--prioritarios)
	Inicia o monitoramento do dispositivos registrados como prioritários e registra relatorios.
...
#		  read -p "Digite quantos dispositivos serão registrados: " dispositivos
		  ...
		  -e|--registrarEmail)
		    until [ "$confirmacao" = "s" ] ; do		#Confirmar as informações
		  	  read -p "Digite o email que será registrado para ser utilizado pelo script (outlook): " email
			  read -p "Digite a senha desse email: " senha
			  read -p "Digite um email para o servidor manter contato: " emailcontato 		 
			  echo -e "\n${vermelho}Confirmação${corpadrao} \nO email será ${ciano}${email}${corpadrao}, a senha será ${ciano}${senha}${corpadrao} e o email de teste será enviado para ${ciano}${emailcontato}${corpadrao}\n"
			  echo "As informações estão corretas? [s/n]"
			  read confirmacao
			  sendemail -f ${email} -t ${emailcontato} -u "Um dispositivo foi desconectado" -m "Olá admin da ${rede}, o dispositivo ${nome_dispositivo} foi desconectado às ${horas} - Ats.MonLan" -s smtp-mail.outlook.com:587 -xu ${email} -xp ${senha}
			  echo "sendemail -f ${email} -t ${emailcontato} -u ªUm dispositivo foi desconectado -m 5Olá admin da ${rede}, o dispositivo ${nome_dispositivo} foi desconectado às ${horas} - Ats.MonLanª -s smtp-mail.outlook.com:587 -xu ${email} -xp ${senha}" >> /$HOME/.monLAN/.email.txt && echo -e "\nRegistro realizado com sucesso. Digite a próxima tarefa: \n"
			done
			confirmacao=""
		  ;;

		  -rp|--registrarPrioritarios) #Realiza o registro dos dispositivos
		    until [ "$confirmacao" = "s" ] ; do		#Confirmar as informações
		  	  read -p "Digite o nome do dispositivo que será registrado como prioritário: " nome
			  read -p "Digite o IP do dispositivo que será registrado como prioritário: " ip     		 
			  echo -e "\n${vermelho}Confirmação${corpadrao} \nO nome será ${ciano}${nome}${corpadrao} e o IP será ${ciano}${ip}${corpadrao}\n"
			  echo "As informações estão corretas? [s/n]"
			  read confirmacao
			  echo "nome:${nome}:IP:${ip}" >> /$HOME/.monLAN/registro_prioritarios.txt && echo -e "\nRegistro realizado com sucesso. Digite a próxima tarefa: \n"
			done
			confirmacao=""
		  ;;

		  -s|--prioritarios)
		  read -p "Como deve ser chamada essa rede que será monitorada?" rede
		  while : #Entra em um loop que faz o teste completo infinitas vezes
		  do
		  echo "" > /$HOME/.monLAN/cache.txt #Esvazia o cache. Pode ser usado um rm também
		  linhas=$(wc -l /$HOME/.monLAN/registroprioritarios.txt | cut -d\  -f 1) #Retira quantas linhas tem o arquivo de registro
		  loop="1"
			until [ "$loop" -gt "$linhas" ]
			  do
			  #Retira apenas o nome do dispositivo e seu IP para fazer o teste
			  nome_dispositivo=$(cat /$HOME/.monLAN/registroprioritario.txt | grep -n ^| grep ^${loop}: | cut -d: -f 3)
			  ip_dispositivo=$(cat /$HOME/.monLAN/registroprioritario.txt | grep -n ^| grep ^${loop}: | cut -d: -f 5)
			  #Realiza o teste de conexão e coloca o resultado no cache
			  horas=$(date | cut -d' ' -f4)
			  envioemail=$(cat /$HOME/.monLAN/.email.txt)
			  ping -c 1 -W 1 ${ip_dispositivo} 1> /dev/null 2> /dev/null && \
			  echo -e "Dispositivo:${ciano}${nome_dispositivo}${corpadrao} Status:${verde}Conectado${corpadrao}" \
			  >> /$HOME/.monLAN/cache.txt || \
			  echo -e "Dispositivo:${ciano}${nome_dispositivo}${corpadrao} Status:${vermelho}Sem-Conexão${corpadrao}" \
			  >> /$HOME/.monLAN/cache.txt && $envioemail
			  #Imprime na tela as informações do cache
			  _LOGO ; cat /$HOME/.monLAN/cache.txt | column -t
			  eval "loop=\$((loop + 1))"
			  done
