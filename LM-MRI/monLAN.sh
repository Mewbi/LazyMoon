  ...
  _LOGO
   read tarefa
      ...
      until [ $tarefa = "--sair" ] 2> /dev/null || [ $tarefa = "-s" ] 2> /dev/null ; do
		  ...
		  do
		  echo -e "${ciano}Monitorando${corpadrao}" > /$HOME/.monLAN/cache.txt #Esvazia o cache. Pode ser usado um rm também
		  ...
		  -rr|--registrar-rede) #Realiza o registro dos dispositivos
			...
			echo "nome-rede:${nome_rede}:IP-inicial:${ip_inicial}:IP-final:${ip_final}" >> /$HOME/.monLAN/registro-rede.txt && echo -e "\nRegistro realizado com sucesso. Digite a próxima tarefa: \n"
			confirmacao=""
		  ;;
		  
		  -cr|--checar-rede)
		  _LOGO
		  tabela=`cut -d: -f 2,4,6 /$HOME/.monLAN/registro-rede.txt | tr -t : \  | column -t`
		  echo -e "${vermelho}\nRede ${ciano}IP-Inicial IP-Final${corpadrao} \n${tabela}" | column -t && echo -e "\nChecagem realizada. Digite a próxima tarefa: \n"
		  ;;

		  -mr|--monitorar-rede)
		    #Selecionando a rede que será monitorada
		    tabela=`cut -d: -f 2,4,6 /$HOME/.monLAN/registro-rede.txt | tr -t : \  | column -t | cat -n`
			...
			read deseja_omitir
			  ...
			  ipinicial=$(cat /$HOME/.monLAN/registro-rede.txt | grep -n ^| grep ^${rede_monitorada} | cut -d: -f 5)
			  ipfinal=$(cat /$HOME/.monLAN/registro-rede.txt | grep -n ^| grep ^${rede_monitorada} | cut -d: -f 7)
		  	  ipatual=$ipinicial
			  ...
			  while : #Entra em um loop que faz o teste completo infinitas vezes
		  	  do
		        echo -e "${ciano}Monitorando${corpadrao}" > /$HOME/.monLAN/cache-rede.txt #Esvazia o cache do monitoramento de rede.
